...
public class HiloServidor extends Thread {
	...
	public HiloServidor(SSLSocket elsocket) {
		//this.socket = socket;
		//this.SSLSocket = SSLSocket;
		this.elsocket = elsocket;
		try {
			fentrada = new DataInputStream(elsocket.getInputStream());
		} catch (IOException e) {
...
// El programa abre un stream de salida para escribir el texto en el socket
	private void EnviarMensajes(String texto) {
		for (int i = 0; i < ServidorChat.CONEXIONES; i++) {
			SSLSocket elsocket = ServidorChat.tabla[i];
			try {
				DataOutputStream fsalida = new DataOutputStream(elsocket.getOutputStream());
